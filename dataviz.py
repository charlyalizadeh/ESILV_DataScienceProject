...
class DataViz:
    ...
    def plot_spectrum(
        self, pixel_index=16, spectrum_index=(0, 1, 2, 3), train=True, **kwargs
    ):
        spectrum_tot = {0: "green", 1: "red", 2: "indra_red1", 3: "infra_red2"}
        mat_value = self.train_set if train else self.test_set
        values = {}

        for index in spectrum_index:
            unique, counts = np.unique(
                mat_value[:, pixel_index + index], return_counts=True
            )
    ...
    def bar_plot_label(self, train=True):
        ...
        mat_value = self.train_set if train else self.test_set
        unique, counts = np.unique(mat_value[:, 36], return_counts=True)
    ...
    def pca(self, train=True, number_components=5):
        ...
        principal_comp_df = pd.DataFrame(
            data=principal_components, columns=["pc_1", "pc_2", "pc_3", "pc_4", "pc_5"]
        )
