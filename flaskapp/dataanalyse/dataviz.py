...
def plot_pixel_value_by_spectrum_index(data, pixel_index=range(9)):
    ...
    for i in range(2):
        for j in range(2):
            plot_pixel_value(pixel_index, [i * 2 + j], labels=False, ax=axs[i, j])
            return fig
...
def plot_pca(data, pca, principal_components, classes):
    ...
    targets = list(data.labels.values())
